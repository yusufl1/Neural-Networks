...
Original file is located at
    https://colab.research.google.com/drive/15ypdnRmr93y5MuIlCcsu-Hg03bMIEOZD
...
class MyGRUCell(nn.Module):
    def __init__(self, input_size, hidden_size):
        ...
        self.Wih = nn.Linear(self.hidden_size, self.input_size)
        self.Win = nn.Linear(self.input_size, self.hidden_size, bias=False)
        ...
        self.Whn = nn.Linear(self.hidden_size, self.hidden_size)
        self.bz = torch.ones(self.hidden_size, 1)
        self.br = torch.ones(self.hidden_size, 1)
        self.bg = torch.ones(self.hidden_size, 1)
...
class AdditiveAttention(nn.Module):
    ...
    def forward(self, queries, keys, values):
        ...
        q = torch.unsqueeze(queries, 1)
        #q = queries.view((queries.shape[0], -1, queries.shape[1]))
        expanded_queries = q.expand(keys.shape)
        
        concat_inputs = torch.cat((expanded_queries, keys), 2)
        unnormalized_attention = self.attention_network(concat_inputs)
        attention_weights = self.softmax(unnormalized_attention)
        
        #context = torch.bmm(torch.reshape(attention_weights, (batch_size, 1, values.shape[1])), values)
        context = torch.sum((attention_weights * values), 1)
...
TEST_SENTENCE = 'pure baby lion snake fright'
translated = translate_sentence(TEST_SENTENCE, rnn_attn_encoder, rnn_attn_decoder, None, args)
print("source:\t\t{} \ntranslated:\t{}".format(TEST_SENTENCE, translated))
...
TEST_SENTENCE = 'why when corner building knife'
translated = translate_sentence(TEST_SENTENCE, transformer_encoder, transformer_decoder, None, args)
print("source:\t\t{} \ntranslated:\t{}".format(TEST_SENTENCE, translated))
